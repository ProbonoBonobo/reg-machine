...
;     ;:env                {:url "" :breakpoint-triggered false :verbose-mode true :trace-mode false :help-mode false  :keydown-events { :total 0  :invalid 0 }}
;     ;:console            {:current-target :r1 :has-update? false :registers {:r1 [] :r2 [] :r3 [] :acquired? false :locked? false :released? false }
;     ;                     :sem {:acquired? true :locked? false :released? false :data [""] }
;     ;                     :config {:log-length 0 :status 0 :breakpoints [""] :append-if [""] :current [""] :log [""]} }
;     ;:accumulator        {:current-target :r1 :registers { :r1 [] :r2 [] :r3 [] :acquired? false :locked? false :released? false }
;     ;                     :sem {:locked? false :acquired? true :released? false :data [""] } }
;     ;:cpu                {:cycle 0
;     ;                     :stack { :input [] :tag [] :response [] :success [] }
;     ;                     :cores [{:id "cpu1" :status 1 :registers {:r1 [] :r2 [] :r3 [] :locked? false :acquired? false :released? false :on-release [""] :owner "" :pid 1}
;     ;                              :sem {:locked? false :acquired? true :released? false :data [""] } }
;     ;                             {:id "cpu2" :status 0 :registers {:r1 [] :r2 [] :r3 [] :locked? false :acquired? false :released? false :on-release [""] :owner "" :pid 2}
;     ;                              :sem {:locked? false :acquired? false :released? false :data [""] } }
;     ;                             {:id "cpu3" :status 0 :registers {:r1 [] :r2 [] :r3 [] :locked? false :acquired? false :released? false :on-release [""] :owner "" :pid 3}
;     ;                              :sem {:locked? false :acquired? false :released? false :data [""] } }
;     ;                             {:id "cpu4" :status 0 :registers {:r1 [] :r2 [] :r3 [] :locked? false :acquired? false :released? false :on-release [""] :owner "" :pid 4}
;     ;                              :sem {:locked? false :acquired? false :released? false :data [""] } }
;     ;                             {:id "cpu5" :status 0 :registers {:r1 [] :r2 [] :r3 [] :locked? false :acquired? false :released? false :on-release [""] :owner "" :pid 5}
;     ;                              :sem {:locked? false :acquired? false :released? false :data [""] } }
;     ;                             {:id "cpu6" :status 0 :registers {:r1 [] :r2 [] :r3 [] :locked? false :acquired? false :released? false :on-release [""] :owner "" :pid 6}
;     ;                              :sem {:locked? false :acquired? false :released? false :data [""] } }
;     ;                             {:id "cpu7" :status 0 :registers {:r1 [] :r2 [] :r3 [] :locked? false :acquired? false :released? false :on-release [""] :owner "" :pid 7}
;     ;                              :sem {:locked? false :acquired? false :released? false :data [""] } }
;     ;                             {:id "cpu8" :status 0 :registers {:r1 [] :r2 [] :r3 [] :locked? false :acquired? false :released? false :on-release [""] :owner "" :pid 8}
;     ;                              :sem {:locked? false :acquired? false :released? false :data [""] } } ]}
;     :data-bus           :result
;     :control-bus        :op
;     :address-bus        :x
...
;                       ;(assoc-in @app-state [:input :label] (js/String character))
;                       ;(assoc-in @app-state [:input :code] (js/String ascii-code)))))
;                       (swap! keyboard-input assoc-in [:key-id] (js/String ascii-code))
;                       (swap! keyboard-input assoc-in [:ascii-code] (.charCodeAt character))
;                       (js/console.log ascii-code)
;                       (cond
...
;           (.getElementById js/document "succulent")))
;(defn quil-setup []
