...
(def db (atom {:mutex      {:state         0
                 ...
                 :semaphoreB {
                               ...
                               :row [:semaphoreB :registers :r3 :val :curr]
                               :col [:semaphoreB :registers :r3 :curr]
                               :locked? true }
                 :opcode 1}))
...
(defn lock [thread]
    (swap! db assoc-in (conj thread :locked?)  true))
...
(defn unlock [thread]
  (do (assert (= (get-in @db [thread :locked?])) (str "Attempted to unlock " thread ", but " thread " is not locked!"))
...
(defn siginterrupt [v]
  (if (odd? (get-opcode)) (do
                            ...
                            (swap! db assoc-in [:semaphoreA :row]
                                   (get (get-in @db [:semaphoreA :available])
                                        (mod (get-in @db [:semaphoreA :state]) 1)))
                            (swap! db assoc-in [:semaphoreA :col]
                                  (get (get-in @db [:semaphoreA :available])
                                              (mod (get-in @db [:semaphoreA :state]) 1)))
                            (swap! db assoc-in [:semaphoreB :row]
                                   (get (get-in @db [:semaphoreB :available])
                                              (mod (get-in @db [:semaphoreB :state]) 3)))
                            (swap! db assoc-in [:semaphoreB :col]
                                   (get (get-in @db [:semaphoreB :available])
                                              (mod (get-in @db [:semaphoreB :state]) 3)))
                            (swap! db update-in [:semaphoreB :locked?] not)
                            (swap! db update-in (resolve (route-to :reg)) str (get-in @keyboard-input [:key-id])))))
...
(def opcode
  (let [input-tag (get-in @keyboard-input [:tag])
  ...
  (vec [
        ;the first action
        (fn [e] (cond
                  (= (get-in @keyboard-input [:tag]) :number) (.log js/console "Can't advance empty register")
                  (= (get-in @keyboard-input [:tag]) :operator) (do
                                                    (swap! db assoc-in (route-to :current-register) (str (get-in e [:key-id])))
                                                    (.log js/console "Put an operator in the op register."))
                  (= (get-in @keyboard-input[:tag]) :equals) (.log js/console "ignoring the equals sign")
                  :else (.log js/console "I don't recognize the argument " (str (get-in e [:key-id])))))

        ;1 push to empty y register
        (fn [e] (cond
                  (= (get-in @keyboard-input  [:tag]) :number)  (do
                                                     (.log js/console "Lock semA and put "  (get-in e [:key-id]) " in next available register")
                                                     (siginterrupt (get-in @keyboard-input [:key-id])))
                  (= (get-in @keyboard-input [:tag]) :operator) (.log js/console "ignoring the operator")
                  (= (get-in @keyboard-input [:tag]) :equals) (.log js/console "ignoring the equals sign")
                  :else (.log js/console "I don't recognize the argument " (str input-tag))))
        ;2 push to empty op
        (fn [e] (cond
                  (= (get-in e [:tag]) :operator) (do
                                                    (.log js/console "Can't advance empty register"))
                  (= (get-in e [:tag]) :number) (do
                                                  (.log js/console (str "Put " (get-in e [:key-id]) " in the current register " (route-to :current-register)))
                                                  (swap! update-in db (resolve (get-route :reg)) str (get-in e [:key-id])  ))
                  (= (get-in e [:tag]) :equals) (.log js/console "ignoring the equals sign")
                  :else (.log js/console "I don't recognize the argument " (str input-tag))))
        ;3 push to a non-empty x-register
        (fn [e] (cond
                  (= (get-in e [:tag]) :operator) (do
                                                    (.log js/console "Lock semB and put " (str (get-in e [:key-id])) " in op reg")
                                                    (swap! db assoc-in [:semaphoreB :locked?] true)
                                                    (swap! db update-in [:mutex :state] bit-xor 1)
                                                    (swap! db assoc-in [:mutex :active-thread] (get (get-in @db [:mutex :threads]) (get-in @db [:mutex :state])))
                                                    (swap! db update-in (route-to :current-register) #(str % (get-in e [:key-id]))))
                  (= (get-in e [:tag]) :number) (do
                                                  (.log js/console (str "Put " (get-in e [:key-id]) " in the current register " (route-to :current-register)))
                                                  (swap! update-in db (resolve (get-route :reg)) str (get-in e [:key-id])))
                  (= (get-in e [:tag]) :equals) (.log js/console "ignoring the equals sign")
...
(defn shortbus [num]
  ...
  (let [current-row (into [] (route-to :current-register))
    ...
    (apply (opcode (resolve (route-to :opcode))) [@keyboard-input])))
;
;    (cond
    ...
    (.addEventListener js/window "keypress"
                   (fn [e]
                     (let [character (.-key e)
                         ...
                         (shortbus (get-in keyboard-input [:tag])))))
(defn render! []
  (.render js/ReactDOM
